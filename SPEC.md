...
> _experience render it with `@chromamark/renderer` or open `examples/demo.html`._
...
On a plain markdown engine a colored block degrades to a blockquote whose first
line is the **bolded title**, followed by the content — or, at worst, the literal
`:::` lines, which are still readable.
:::
...
- **token-or-color** — a semantic token (§4) or `color=`/hex/name.
...
Fallback: maps to `<details><summary>` where available; in a plain engine, the
summary shows as bold text with the content beneath it — content is **never**
hidden in a non-supporting renderer (principle 3).
...
| ------------- | ------------------- | -------------------------------------- |
| Colored block | `::: warning Title` | blockquote with **bold title** + body  |
| Pill          | `[!success PASS]`   | literal text `[!success PASS]`         |
| Collapsible   | `::: details Sum…`  | **bold summary** + always-visible body |
| Colored text  | `[.danger x]`       | literal text `[.danger x]`             |
| Meter         | `[=success 87%]`    | literal text `[=success 87%]`          |
| Fields        | `::: fields`        | blockquote of `key: value` lines       |

Nothing is ever _hidden_ or _lost_ when unsupported.
...
colorval     = "#" , 6*hexdig | named-color | tone ;

pill         = "[" , "!" , spec , [ ws , label ] , "]" ;
text         = "[" , "." , spec , ws , label , "]" ;
meter        = "[" , "=" , spec , ws , value , "]" ;
spec         = tone | "color=" , colorval | colorval ;
label        = { any-char-except-"]" } ;
value        = number , [ "%" ] | number , "/" , number | label ;
...
Demo: `examples/demo.html` — a rendered reference of every construct with a light/dark toggle.
:::
