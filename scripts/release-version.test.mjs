...
test('v0.6.1 coordinated package versions are aligned', () => {
  ...
  assert.match(text('packages/python/src/chromamark/__init__.py'), /^__version__ = "0\.4\.0"$/m);
});
...
test('changelog contains the VS Code v0.4.1 release', () => {
  assert.match(text('CHANGELOG.md'), /^## \[VS Code 0\.4\.1\] - \d{4}-\d{2}-\d{2}$/m);
});

test('changelog contains the VS Code v0.4.0 release', () => {
  assert.match(text('CHANGELOG.md'), /^## \[VS Code 0\.4\.0\] - \d{4}-\d{2}-\d{2}$/m);
});
...
test('changelog contains the VS Code v0.2.4 release', () => {
  assert.match(text('CHANGELOG.md'), /^## \[VS Code 0\.2\.4\] - 2026-07-11$/m);
});
...
test('changelog contains the coordinated v0.6.1 release', () => {
  ...
  assert.match(changelog, /^\[0\.6\.1\]: .*\/releases\/tag\/v0\.6\.1$/m);
});

test('changelog contains the coordinated v0.6.0 release', () => {
  const changelog = text('CHANGELOG.md');
  assert.match(changelog, /^## \[0\.6\.0\] - \d{4}-\d{2}-\d{2}$/m);
  assert.match(changelog, /^\[0\.6\.0\]: .*\/releases\/tag\/v0\.6\.0$/m);
});
...
test('changelog contains the coordinated v0.4.3 release', () => {
  const changelog = text('CHANGELOG.md');
  assert.match(changelog, /^## \[0\.4\.3\] - \d{4}-\d{2}-\d{2}$/m);
  assert.match(changelog, /^\[0\.4\.3\]: .*\/releases\/tag\/v0\.4\.3$/m);
});
