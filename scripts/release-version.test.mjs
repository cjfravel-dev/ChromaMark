...
test('v0.4.0 coordinated package versions are aligned', () => {
  ...
  assert.equal(vscode.version, '0.2.0');
  assert.equal(cli.dependencies['@chromamark/renderer'], '^0.4.0');
  assert.equal(vscode.dependencies['@chromamark/renderer'], '^0.4.0');

  const pyproject = text('packages/python/pyproject.toml');
  assert.match(pyproject, /^version = "0\.2\.0"$/m);
  assert.match(text('packages/python/src/chromamark/__init__.py'), /^__version__ = "0\.2\.0"$/m);
});
...
test('package lock records every coordinated npm version', () => {
  ...
  assert.equal(lock.packages['packages/renderer'].version, '0.4.0');
  assert.equal(lock.packages['packages/cli'].version, '0.3.0');
  assert.equal(lock.packages['packages/conformance'].version, '0.1.0');
  assert.equal(lock.packages['packages/vscode'].version, '0.2.0');
});

test('changelog contains the coordinated v0.4.0 release', () => {
  const changelog = text('CHANGELOG.md');
  assert.match(changelog, /^## \[0\.4\.0\] - 2026-07-10$/m);
  assert.match(changelog, /^\[0\.4\.0\]: .*\/releases\/tag\/v0\.4\.0$/m);
});
