...
test('v0.3.1 coordinated package versions are aligned', () => {
  ...
  assert.match(pyproject, /^version = "0\.1\.4"$/m);
  assert.match(text('packages/python/src/chromamark/__init__.py'), /^__version__ = "0\.1\.4"$/m);
});
...
test('changelog contains the coordinated v0.3.1 release', () => {
  const changelog = text('CHANGELOG.md');
  assert.match(changelog, /^## \[0\.3\.1\] - 2026-07-09$/m);
  assert.match(changelog, /^\[0\.3\.1\]: .*\/releases\/tag\/v0\.3\.1$/m);
});
