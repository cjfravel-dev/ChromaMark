...
export default function containerPlugin(md, enabled) {
  ...
  md.renderer.rules.cm_container_open = (tokens, idx) => {
    ...
    // Custom render rules bypass markdown-it's default token renderer, so any
    // attributes a consumer set on the token have to be emitted here or they are
    // silently dropped. Renders empty unless something set them.
    const attrs = md.renderer.renderAttrs(tokens[idx]);
    if (meta.structure === 'details') {
      const open = meta.open ? ' open' : '';
      return (
        `<details class="cm-details${custom}"${tone}${style}${attrs}${open}>` +
