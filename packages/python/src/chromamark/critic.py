...
def _find_close(state, needle, frm):
    """First index of the close token at or after `frm`, memoized per token on the
    parse state so many unterminated openers stay O(n) instead of O(n^2)."""
    slot = "_cm_critic_" + needle
...
def _rule(state, silent):
    ...
    close_idx = _find_close(state, close, content_start)
